...
function SPangDe:playDisplay1(co, logic)
    ...
    if self.mAttackTimes == 1 then
        ...
        self:playEffectOnce("pangde", "attack_1", self:getPos(3))
    ...
    elseif self.mAttackTimes == 2 then
        ...
        co:waitForEvent(SK_EVENT.Frame_Event, model)
        logic:resume("step1")
        self:playEffectOnce("pangde", "attack_2", self:getPos(3))
        co:waitForEvent(SK_EVENT.Frame_Event, model)
        logic:resume("step2")
        co:waitForEvent(SK_EVENT.Movement_Complete, model)
    else
        self:playModelAnimate(model, "attack_3")
        self:playEffectOnce("pangde", "attack_3_1", self:getPos(3))
        self:playEffectOnce("pangde", "attack_3_2", self:getPos(3), -1)
        co:waitForEvent(SK_EVENT.Frame_Event, model)
        co:waitForEvent(SK_EVENT.Frame_Event, model)
        logic:resume("step1")
        co:waitForEvent(SK_EVENT.Frame_Event, model)
...
function SPangDe:playDisplay2(co, logic)
    ...
    self:playEffectOnce("pangde", "effect_2", self:getPos(3))
    co:waitForEvent(SK_EVENT.Frame_Event, model)
    logic:resume("step1")
    co:waitForEvent(SK_EVENT.Movement_Complete, model)
