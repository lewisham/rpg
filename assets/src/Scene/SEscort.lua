...
local SEscort = class("SEscort", Root)

-- 初始化
function SEscort:init(args)
...
-- 开始执行逻辑
function SEscort:play(co)
    local list = {}
    -- 进场
    self:createComponent("Root.Battle.UI.UILoading", {10001, 10002, 10003, 10004})
    co:waitForFuncResult(function() return self:getComponent("UILoading"):isDone() end)
...
function SEscort:initPlayerMonsters()
...
-- 初始化数据
function SEscort:initEnemyMonsters()
...
-- 战斗
function SEscort:playFighting(co)
    self:getComponent("UIScene"):cameraMoveTo(0, 0.1)
